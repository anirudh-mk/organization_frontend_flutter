...
class VendorCreatePage extends StatefulWidget {
  ...
  const VendorCreatePage({super.key, this.vendor});
...
class _VendorCreatePageState extends State<VendorCreatePage> {
  ...
  void initState() {
    ...
    if (_isEditing) {
      ...
      for (var addrObj in v.addressList) {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_isEditing) {
        await _service.updateVendor(widget.vendor!.id, data);
      } else {
        await _service.createVendor(data);
      }

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_isEditing ? "Vendor updated!" : "Vendor created!"),
